...
export default function RootLayout({ children }) {
  ...
  const [headerPostcodeBtnDisplay, setHeaderPostcodeBtnDisplay] =
    useState(true);
  ...
  const [isdeliverychangedbtnclicked, setIsdeliverychangedbtnclicked] =
    useState(false);
  const [iscartfull, setIscartfull] = useState(true);

  const [iscartitemdottedbtnclicked, setIscartitemdottedbtnclicked] =
    useState(false);
  ...
  useEffect(() => {
    async function fetchWebsiteImageColor() {
      try {
        const response = await axiosPrivate.get(
          `/website-modification-detail/${BRAND_GUID}/${PARTNER_ID}`
        );
        if (
          response?.data?.data?.websiteModificationLive !== null &&
          response?.data?.data?.websiteModificationLive?.json_log[0]
            ?.websiteLogoUrl !== null
        ) {
          setBrandlogo(
            response?.data?.data?.websiteModificationLive?.json_log[0]
    ...
    const getSelectStore = window.localStorage.getItem(
      `${BRANDSIMPLEGUID}user_selected_store`
    );
    if (getSelectStore === null) {
      if (
        pathnameArray[0] === "track-order" ||
        pathnameArray[0] === "review-order" ||
        pathnameArray[0] === "payment" ||
        pathnameArray[0] === "place-order"
      ) {
        setAtfirstload(false);
      ...
      setPostcode(
        JSON.parse(
          window.localStorage.getItem(`${BRANDSIMPLEGUID}user_valid_postcode`)
        )
      );
      ...
      const appliedAmountDiscount = JSON.parse(
        window.localStorage.getItem(
          `${BRANDSIMPLEGUID}order_amount_discount_applied`
        )
      );
      const address = JSON.parse(
        window.localStorage.getItem(`${BRANDSIMPLEGUID}address`)
      );
      const getDeliveryMatrix = JSON.parse(
        window.localStorage.getItem(`${BRANDSIMPLEGUID}delivery_matrix`)
      );
      ...
      const cartDataFromLocalStorage = JSON.parse(
        window.localStorage.getItem(`${BRANDSIMPLEGUID}cart`)
      );

      setCartdata(
        cartDataFromLocalStorage === null ? [] : cartDataFromLocalStorage
      );
    }
